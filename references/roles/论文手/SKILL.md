...
10. 完成下列确定性门禁后，派发独立质检 Subagent 执行 `W2` 论文终检；验收时同时加载 `references/评委全维度终审.md`，从摘要与第一信息面、题意与结构、数学表达、算法叙述、结果与图表、验证/误差/稳健性、模型评价、创新真实性、数据与文献、附录复现、格式匿名合规和全文一致性进行独立终审；未返回 `PASS` 不得宣称完成或交付论文。
