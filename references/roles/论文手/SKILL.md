...
10. 完成下列确定性门禁后，派发独立质检 Subagent 执行 `W2` 论文终检；验收时同时加载 `references/评委全维度终审.md`，从摘要与第一信息面、题意与结构、数学表达、算法叙述、结果与图表、验证/误差/稳健性、模型评价、创新真实性、数据与文献、附录复现、格式匿名合规和全文一致性进行独立终审；若目标为 CUMCM 2026，则仅在此阶段再加载 `../../../references/contests/cumcm/2026/README.md`；未返回 `PASS` 不得宣称完成或交付论文。
...
| CUMCM 2026 W2 与最终交付 | `../../../references/contests/cumcm/2026/README.md` |
| 阶段内独立验收 | `../../../references/Subagent调度.md`；执行 `W2` 时同时读取 `references/评委全维度终审.md` |
