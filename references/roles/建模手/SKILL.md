...
7. 立即派发未参与编写的质检 Subagent 执行 `M1` 建模终检；验收时同时加载 `references/模型适配性终检.md`，重点复核问题—模型适配、复杂度必要性、创新真实性、假设与约束、参数可确定性、验证设计、适用边界和可实现性；未返回 `PASS` 不得进入编程阶段或宣称建模交付完成。
...
| 独立验收 | `../../../references/Subagent调度.md`；执行 `M1` 时同时读取 `references/模型适配性终检.md` |
