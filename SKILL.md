...
本 Skill 用三个独立角色完成数学建模。各阶段交付物应按照当前任务的建模、复现、证据、格式和独立验收门禁生成到可供团队最终审阅的高质量候选终稿；是否提交由参赛队在完成当届官方要求的人工核验、必要修改和合规确认后决定。论文结构与格式必须以目标竞赛当届官方规则和官方模板为准，不能用往届经验替代官方要求。

**使用边界**：首次部署或用户需要了解交付边界时读取根目录 `使用指南.md`。该文件属于 Skill 说明文档，不作为正常比赛项目的运行时输入，不要求复制到 `PROJECT_ROOT`。真实竞赛的具体提交、AI 和支撑材料要求只在对应阶段按当届官方规则加载。
...
Shell、Python、MATLAB、LaTeX 和配置文件中的代码与命令按字面语义处理。不得因 Markdown、富文本或 LaTeX 显示规则自行插入转义字符；尤其不得把普通下划线 `_` 改写为 `\_`。

若用户消息中的代码疑似被界面转义，先根据代码语法和项目文件确认实际字面量，再执行；不得把明显的显示转义直接写入源代码或命令。

对于较长命令或程序，优先写入项目中的 `.py`、`.m`、`.sh` 或其他源码文件再执行，避免在聊天输入层反复重写长代码。
